...
> **Note:** testmynids.org uses CloudFront CDN — different requests may hit different IPs
> (13.35.20.57, .59, .90, .97). Each unique IP is blocked once and skipped on repeat.
...
| Same IP blocked multiple times in iptables | Script now checks before inserting — run `sudo iptables -F INPUT` to clear duplicates |
